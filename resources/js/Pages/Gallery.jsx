                    ...
                    {galleryItems.length > 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            {galleryItems.map((item, index) => (
                                <div key={item.id || index} className="group flex flex-col rounded-2xl overflow-hidden shadow-lg border border-gray-100 hover:shadow-2xl transition-all duration-300">
                                    <div className="aspect-video bg-gray-200 relative overflow-hidden">
                                        {item.images && item.images.length > 0 ? (
                                            ...
                                            <div className="w-full h-full flex items-center justify-center bg-brand-neutral-200 text-brand-neutral-400">
                                    ...
                                    <div className="p-6 bg-white flex flex-col grow">
                                        <h3 className="font-eina font-bold text-xl text-brand-neutral-900 mb-2 group-hover:text-brand-primary transition-colors">{item.title}</h3>
                                        {item.description && (
                                            <p className="font-eina font-light text-brand-neutral-500 text-sm line-clamp-3">{item.description}</p>
                                        )}
                                    </div>
                                </div>
                            ))}
